This zip file contains 4 folders:
  "Data" folder: contains the dataset used and a ReadMe file explaining the dataset used.
  "Proposal" folder: contains a PDF file laying out goals and methodology used to create the project.
  "Project": folder: both the RMD and HTML files for my report can be found here.
  "Presentation" folder: Has the PowerPoint slides used to present findings from the report.

